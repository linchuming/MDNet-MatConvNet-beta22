...
net.layers{end}.weights = { ...
    0.01 * randn(1,1,512,2*K,'single'), ...
    zeros(1, 2*K, 'single')};
net.layers{end+1} = struct('type', 'softmaxloss_k', 'name', 'loss','precious', true) ;
...
net.layers{end+1} = struct('type', 'conv', ...
    ...
    'biasesWeightDecay', 0) ;
net.layers{end}.weights = { ...
    0.01 * randn(1,1,512,2,'single'), ...
    zeros(1, 2, 'single')};
net.layers{end+1} = struct('type', 'softmaxloss', 'name', 'loss','precious', true) ;
net = vl_simplenn_tidy(net);
