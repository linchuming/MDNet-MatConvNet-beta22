...
for To = 2:nFrames;
    ...
    target_score = mean(scores(1));
    fprintf('target_score: %f...', target_score);
    score(To) = target_score;
    % targetLoc = round(mean(samples(idx(1:5),:)));
    targetLoc = round(samples(idx(1),:));
...
%         fprintf('\n');
    ...
    if display
        ...
        print(gcf, '-dpng', ['res/', int2str(To), '.png']);
    end
end
% save('scores.mat', 'score');